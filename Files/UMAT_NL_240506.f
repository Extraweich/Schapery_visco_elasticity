...
SUBROUTINE UMAT(stress,statev,ddsdde,sse,spd,scd,&
	...
	DOUBLE PRECISION :: LAMBDA, MU, ind_stress
    ...
    IF (time(2) == 0.D0) THEN
      ! assign non-linear parameters in beginning of simulation
      G0_NL = 1.D0
      G1G2_NL = 1.D0
      A_SIG_NL = 1.D0
    ELSE
	...
	IF (DEBUG == .TRUE.) THEN
	  ...
	  print *, 'IND_STRESS', GET_IND_STRESS(stress+DSIGMA_TRIAL)
	END IF
	
    
    ! Calculate trial strain to check if trial stress is sufficient
    DSTRAN_TRIAL = GET_DEPS(stress, DSIGMA_TRIAL, STRAN_INH1, STRAN_INH2, STRAN_INH3, &
	  dtime, ntens, G0, G1, G2, G3, K0, K1, K2, K3, TAU1, TAU2, TAU3, M_PRIME_EPS, M_PRIME_SIG, M_CIRC)
    
    ! Calculate residuum
    STRAN_RES = GET_RES_NORM(DSTRAN_TRIAL, dstran, ntens, DEBUG)
	...
	IF (ITER == ITER_MAX .OR. STRAN_RES > RES_MIN) THEN
	...
	ind_stress = GET_IND_STRESS(stress)
    ...
    STATEV(22) = STRAN_RES
	STATEV(23) = ind_stress
	
	IF (DEBUG == .TRUE.) THEN
	  j = 0
	  DO I=19, 23
	    IF (STATEV(I) /= STATEV(I)) THEN
	      j = 1
	      print *, 'StateV(I) is NaN, I, IND_STRESS, time = ', I, IND_STRESS, time(1)
	    END IF
	  END DO
...
!
	...
	! Indicator stress (should be equal to Abaqus v.Mises)
	STATEV(23) = 0.D0
